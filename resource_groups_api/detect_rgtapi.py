...
def get_tagging_api_client(region: str = "eu-west-2"):
    ...
    if boto3 is None:
        raise RuntimeError("Optional dependency missing: boto3. Install requirements-rgtapi.txt to use this script.")
    try:
...
def discover_tagged_resource_types(client) -> dict:
    ...
    try:
        ...
        if error_code == "PaginationTokenExpiredException":
            console.print("[yellow]Pagination token expired (large account). Returning partial results.[/yellow]")
            return {k: sorted(list(v)) for k, v in resource_types.items()}
